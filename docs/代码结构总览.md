...
└── api/
    ...
    ├── search.rs        # POST /search → 混合检索（keyword + vector RRF），向量侧失败降级 keyword
    ├── graph.rs         # GET /graph，解析 wikilink 建图
    ├── runtime.rs       # GET /runtime-config（chatEnabled、llmConfig 摘要）
    └── chat.rs          # POST /chat SSE（reqwest 直连 LLM，不再 spawn node）
llm.rs                   # embed_query + stream_chat + <think> 分割器 + SSE 行解析（chat 搜索共用）
```
...
**Chat 为何单独分支：** JSON 路由（`handle_request`）返回的是一次性 `ApiResponse`，无法逐帧写出 SSE；`handle_request` 内对 `POST .../chat` 故意返回 501，强制走 `server.rs` 的 SSE 分支（`try_handle_chat_sse`）。Chat 现在在 Rust 进程内用 reqwest 直接拉 LLM 的 SSE 流，不再 spawn Node 子进程。
...
### 5.4 Chat SSE 细节（`api/chat.rs` + `llm.rs`）

1. 鉴权、`resolve_project(project_id)`
2. 要求 `LLM_WIKI_CONFIG` 含 `llmConfig`（`apiMode` 须为 `chat_completions`，否则 400）
3. 校验 body：`{ "messages": [ { "role", "content" }, ... ] }`
4. cookie 用户走每日配额检查；占用 `MAX_CONCURRENT_CHAT=8` 并发槽
5. `runtime.block_on(stream_chat(...))`：reqwest POST `customEndpoint + /chat/completions`，`stream:true`
6. 逐行解析上游 SSE（`parse_sse_line`）→ `choices[0].delta.content` / `reasoning_content` / `[DONE]`
7. content 经 `ThinkSplitter`（`<think>` 跨 token 状态机）拆成 `token`/`reasoning` 帧
8. `ChatResponder` 写 chunked `data: {"event","data"}\n\n`，每帧 flush；客户端断连则 `is_alive=false` 停止拉取

SSE 线格式与旧 Node 版完全一致（`overlay/web/lib/llm-client.ts` 解析 `parsed.event`/`parsed.data`），前端无需改动。仅支持 OpenAI 兼容（`chat_completions`）线路；`anthropic_messages`/`minimax` 等返回明确错误。
...
├── Cargo.toml        # 依赖 lancedb / arrow / futures（编译需 protoc）
└── src/
    ├── lib.rs
    ├── project.rs     # normalize_path, resolve_project_dir, wiki_dir, sources_dir
    ├── rescan.rs      # rescan_project：遍历 raw/sources 算 md5
    ├── vector.rs      # LanceDB 读写：upsert_chunks / search_vector / count_chunks（CLI ingest 与 server 共用）
    ├── hybrid.rs      # RRF 融合纯逻辑（merge_rrf / RankedItem，无 IO）
    └── search/
        ├── mod.rs     # hybrid_search：keyword + vector 融合编排，向量侧失败吞错降级
...
| `search` | `search_keyword`, `hybrid_search`, `ProjectSearchResponse` | **server** `api/search.rs`、**CLI** `cmd_search.rs` |
| `vector` | `upsert_chunks`, `search_vector`, `count_chunks` | CLI `cmd_vector`/ingest、server 间接经 `hybrid_search` |
| `hybrid` | `merge_rrf`（RRF 融合） | `search::hybrid_search` |

**搜索算法概要：**
- **关键词**：对 query 分词（含 CJK 二元 gram），在 `wiki/` 下 `.md` 标题与正文匹配，加权得分，UTF-8 安全 snippet。
- **向量**：`search_vector` 用 LanceDB `nearest_to` 近邻查询 `wiki_chunks_v2` 表，按 `_distance` 升序返回。
- **混合**：`hybrid_search` 同时跑两路，`merge_rrf`（k=60 RRF）融合去重，`mode` 为 `hybrid`/`keyword`。**向量侧任何失败**（无 embedding、无表、维度不符、embed 接口不可达）都被吞掉并降级为纯关键词，HTTP 仍返回 200 结果。`page_id`（= 文件名去 `.md`）经 `walk wiki/` 映射回 relpath。
...
│       ├── cmd_rescan.rs
│       ├── cmd_reindex.rs
│       ├── cmd_ingest.rs         # drive Node cmd-ingest.ts（node tsx cli.mjs，非 npx）
│       └── cmd_vector.rs         # 隐藏：LanceDB upsert/delete/count（→ common::vector）
└── node/
    └── src/
        ├── cmd-ingest.ts         # → upstream autoIngest()
        ├── cmd-reindex.ts        # → upstream embedAllPages()
        ├── cmd-llm-stream.ts     # 【已弃用】旧 chat SSE 脚本；server 现用 reqwest 直连，不再调用
        ├── load-config.ts
...
| `llm-wiki ingest` | Rust drive Node | `cmd_ingest::run` → `cmd-ingest.ts` → `autoIngest` |
| `llm-wiki vector *` | Rust → common::vector | 由 upstream embedding 经 shim `invoke` 调用 |
| HTTP `POST /search` | Server + common | `api/search` → `hybrid_search`（keyword+vector RRF） |
| HTTP `POST /chat` | Server reqwest | `llm::stream_chat` 直连 LLM（**不再 spawn Node**） |
...
因此 **ingest / embedding** 复用 upstream 同一份 TS，无需 fork。**Chat 不再走 Node**——server 用 reqwest 直连 OpenAI 兼容端点，`cmd-llm-stream.ts` 已不再被调用（保留为死代码，可在后续清理时删除）。
...
./scripts/llm-wiki ingest FILE --project PATH --config CONFIG
  ...
  overlay/cli/rust/cmd_ingest.rs    → 校验 project/source/config，canonicalize config 路径
                                    → node <tsx cli.mjs> overlay/cli/node/src/cmd-ingest.ts
                                      （直接 drive node + tsx CLI 模块，非 npx，避免 batch hang）
...
sequenceDiagram
  ...
  Rust->>Node: node tsx/cli.mjs cmd-ingest.ts
...
sequenceDiagram
  ...
  UI->>API: POST /search (RAG，混合检索)
  API-->>UI: wiki snippets (keyword+vector)
  UI->>API: POST /chat SSE
  API->>LLM: reqwest POST /chat/completions (stream)
  loop tokens
    LLM-->>API: SSE delta (content/reasoning_content)
    API->>API: ThinkSplitter 拆 <think>
    API-->>UI: data {event:token|reasoning}
  end
  LLM-->>API: [DONE]
...
| POST | `/projects/{id}/search` | `search.rs` | 混合检索（keyword + vector RRF），向量侧失败降级 keyword |
| GET | `/projects/{id}/graph` | `graph.rs` | wikilink 图 |
| GET | `/runtime-config` | `runtime.rs` | UI 用 LLM 摘要 |
| POST | `/projects/{id}/chat` | **`chat.rs`（SSE）** | 非 JSON 路由，reqwest 直连 LLM（无 Node） |
| POST | `/projects/{id}/sources/rescan` | `mod.rs` | **501**（用 CLI rescan） |

**SSE 事件格式**（`api/chat.rs::build_sse_frame`，与旧 Node 版一致）：`data: {"event":"token|done|error|reasoning","data":{...}}\n\n`
