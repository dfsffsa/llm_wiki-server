...
| Chat | WebView TS（桌面内） | **HTTP SSE**（`overlay/server/src/api/chat.rs` + Node `cmd-llm-stream.ts`）；Lite 页 `/lite/` |

上游桌面自带的 `:19828` API 中，`POST .../chat` 在官方树里仍可能返回 501；**本集成仓库的 `llm-wiki-server` 已实现 Chat 代理**（在 `server.rs` 中先于通用路由 dispatch）。完整版 React UI 与 Lite 静态页均通过该端点流式对话。
