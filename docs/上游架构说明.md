...
> 原文基于仓库 **0.4.14** 调研编写；本集成仓库 upstream submodule 当前为 **v0.4.20**（2026-06-06 验证）。  
...
| 能力 | 桌面 upstream | llm_wiki-server overlay（2026-06-06） |
|------|---------------|--------------------------------------|
| 浏览 wiki | GUI | Web + HTTP |
| Ingest | GUI + 队列 | **CLI**（`llm-wiki ingest`） |
| 搜索 | GUI + Rust | HTTP API + **CLI** |
| Chat | WebView TS（桌面内） | **HTTP SSE**（`overlay/server/src/api/chat.rs` + `llm.rs`，reqwest 直连 LLM，无 Node）；Lite 页 `/lite/` |
| 向量搜索 | GUI + Rust（LanceDB） | HTTP `/search` 混合检索（keyword + vector RRF，向量侧失败降级） |

上游桌面自带的 `:19828` API 中，`POST .../chat` 在官方树里仍可能返回 501；**本集成仓库的 `llm-wiki-server` 已实现 Chat 代理**（在 `server.rs` 中先于通用路由 dispatch），且为纯 Rust reqwest 实现（不再 spawn Node 子进程）。完整版 React UI 与 Lite 静态页均通过该端点流式对话。

完整改造计划见 [架构与改造方案.md](./架构与改造方案.md)。
