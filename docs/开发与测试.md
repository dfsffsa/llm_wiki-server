...
> **低配服务器（内存 ≤ 2GB / 磁盘 I/O 差）无法直接编译 CLI**：链接 `lance/lancedb` 阶段会 OOM 或耗时数小时。改在本地机器上交叉编译 musl 静态二进制后 scp 上去。详见 **[低配机交叉编译CLI.md](./低配机交叉编译CLI.md)**。
...
| 120 秒超时 | 单次问答保护；可检查 `llmConfig`（endpoint/model/apiKey）。Chat 已是 Rust reqwest 直连，排查看 server stderr 日志，不再有 Node 子进程 |
